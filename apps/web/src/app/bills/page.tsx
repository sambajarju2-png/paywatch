...
/* Actual DB status values: outstanding, action, settled, failed, review */
const tabs = ["Openstaand", "Actie nodig", "Betaald", "Alle"];
const tabFilters: Record<string, string[]> = {
  Openstaand: ["outstanding"],
  "Actie nodig": ["action", "failed", "review"],
  Betaald: ["settled"],
  Alle: ["outstanding", "action", "settled", "failed", "review"],
};
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        .from("bills")
        .select("id, vendor, amount, due_date, status, escalation_stage, category")
        .eq("user_id", user.id)
  ...
  return (
    <div className="px-4 py-4 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowAdd(!showAdd)} className="rounded bg-[var(--blue)] px-3 py-1.5 text-xs font-semibold text-white">+ Toevoegen</button>
      </div>
        ...
        {tabs.map((tab, i) => {
          ...
          return (
            <button key={tab} onClick={() => setActiveTab(i)}
              className={`flex-1 rounded-md py-1.5 text-[10px] font-semibold transition-colors ${activeTab === i ? "bg-[var(--surface)] text-[var(--text)] shadow-sm" : "text-[var(--muted)]"}`}>
      ...
      {loading ? (
        <div className="flex flex-col gap-2 animate-pulse">{[1,2,3,4].map(i => <div key={i} className="h-16 bg-[var(--border)] rounded-xl opacity-40" />)}</div>
      ) : filtered.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {filtered.map((bill) => {
            const isAction = bill.status === "action" || bill.status === "failed";
            const color = stageColors[bill.escalation_stage] || "var(--blue)";
            return (
              <div key={bill.id} className="flex items-center gap-3 rounded-xl border p-3"
                style={{
                  borderColor: isAction ? "color-mix(in srgb, var(--red) 20%, transparent)" : "var(--border)",
                  background: isAction ? "color-mix(in srgb, var(--red) 4%, transparent)" : "var(--surface)",
                }}>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-[var(--text)] truncate">{bill.vendor}</p>
                ...
                <div className="text-right flex-shrink-0">
                  <p className="text-sm font-bold text-[var(--text)]">{fmt(bill.amount)}</p>
...
function AddBillForm({ onClose, onAdded }: { onClose: () => void; onAdded: (bill: Bill) => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const billId = `manual_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    ...
    if (data && !error) onAdded(data as Bill);
    setSaving(false);
  }
  ...
  return (
    <div className="rounded-xl border border-[var(--blue)] bg-[var(--surface)] p-4">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-2.5">
        ...
        <button type="submit" disabled={saving} className="w-full rounded-lg bg-[var(--blue)] py-2 text-sm font-semibold text-white disabled:opacity-50">
