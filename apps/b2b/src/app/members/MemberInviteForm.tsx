  ...
  async function handleInvite(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/members", {
        ...
        body: JSON.stringify({ email, full_name: fullName.trim() || null, role, organization_id: orgId }),
      });
      ...
      <form onSubmit={handleInvite} className="space-y-3">
        <div className="flex gap-3">
          <div className="flex-1">
            <label className="block text-xs font-semibold text-pw-muted uppercase tracking-wider mb-1">Volledige naam</label>
            <input
              type="text"
              value={fullName}
