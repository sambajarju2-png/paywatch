...
import { getComparisonBySlug, comparisons, escalationSteps, getCommonalities, getFAQs } from "@/lib/comparison-data";
...
/* ─── Main Page ─── */
export default function ComparisonContent({ slug }: { slug: string }) {
  const { lang } = useApp();
  ...
  return (
    <div className="bg-[var(--bg)]">
      ...
      {/* ─── What they have in common (reduces bounce) ─── */}
      <section className="mx-auto max-w-4xl px-4 pb-12 sm:px-6">
        <ScrollReveal>
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6">
            <h2 className="text-lg font-bold text-[var(--navy)] mb-4">
              {lang === "nl" ? `Wat hebben ${data.name} en PayWatch gemeen?` : `What do ${data.name} and PayWatch have in common?`}
            </h2>
            <ul className="space-y-2.5">
              {getCommonalities(data)[lang].map((item, i) => (
                <li key={i} className="flex items-start gap-2.5 text-sm text-[var(--text)]">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--green)" strokeWidth="2.5" strokeLinecap="round" className="shrink-0 mt-0.5"><polyline points="20 6 9 17 4 12"/></svg>
                  {item}
                </li>
              ))}
            </ul>
      ...
      <section className="mx-auto max-w-4xl px-4 pb-12 sm:px-6">
        <ScrollReveal>
          <div className="rounded-2xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900 p-6 text-center">
            <p className="text-base font-semibold text-[var(--navy)] mb-1">
              {lang === "nl"
                ? `${pwFeatureCount} van ${data.features.length} functies — probeer PayWatch gratis`
                : `${pwFeatureCount} of ${data.features.length} features — try PayWatch free`}
            </p>
            <p className="text-sm text-[var(--muted)] mb-4">
              {lang === "nl"
                ? "Verbind je inbox en zie direct welke rekeningen aandacht nodig hebben"
                : "Connect your inbox and see which bills need attention right away"}
            </p>
            <a href="https://app.paywatch.app" className="inline-flex items-center gap-2 rounded-xl bg-[var(--blue)] text-white px-5 py-2.5 text-sm font-semibold shadow-[0_4px_14px_rgba(37,99,235,0.3)] hover:shadow-[0_6px_20px_rgba(37,99,235,0.4)] transition-all active:scale-[0.97]">
      ...
      <section className="mx-auto max-w-4xl px-4 pb-12 sm:px-6">
        <ScrollReveal>
          <h3 className="text-base font-bold text-[var(--navy)] mb-3">
            {lang === "nl" ? "Ontdek PayWatch functies" : "Discover PayWatch features"}
          </h3>
          <div className="flex flex-wrap gap-2">
            {[
              { href: "/features/email-scanner", nl: "E-mail scanner", en: "Email scanner" },
              { href: "/features/camera-scanner", nl: "Camera scanner", en: "Camera scanner" },
              { href: "/features/betaalfases", nl: "Escalatie tracking", en: "Escalation tracking" },
              { href: "/features/buddy-systeem", nl: "Buddy systeem", en: "Buddy system" },
              { href: "/features/schuldhulp", nl: "Schuldhulp zoeker", en: "Debt help finder" },
              { href: "/schuldhulp", nl: "Schuldhulp per gemeente", en: "Debt help by municipality" },
            ].map((link) => (
              <Link key={link.href} href={link.href} className="rounded-full border border-[var(--border)] bg-[var(--surface)] px-3.5 py-1.5 text-xs font-medium text-[var(--text)] hover:border-[var(--blue)] hover:text-[var(--blue)] transition-colors">
                {link[lang]}
              </Link>
            ))}
          </div>
