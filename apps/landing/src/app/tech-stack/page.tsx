...
import {
  Mail, Shield, Database, Smartphone, Rocket, BarChart3,
  Plug, Globe, Code2, Server, Zap, Lock, Eye, Layers, GitBranch,
  ChevronDown, ExternalLink, ScanSearch,
} from "lucide-react";
...
const DARK_ICONS = new Set(["nextjs", "vercel", "github"]);
...
const PRINCIPLES = [
  ...
  { icon: <Eye size={18} />, title: { nl: "Jij houdt de controle", en: "You stay in control" }, desc: { nl: "PayWatch analyseert je rekeningen maar jij bevestigt altijd. Geen ondoorzichtige beslissingen over jouw geld.", en: "PayWatch analyzes your bills but you always confirm. No opaque decisions about your money." } },
...
export default function TechStackPage() {
  ...
  /* Data flow steps */
  const FLOW = [
    { step: "1", title: isNl ? "Email binnenkomst" : "Email arrives", desc: isNl ? "Je inbox wordt dagelijks gescand" : "Your inbox is scanned daily", icon: <Mail size={16} />, color: "var(--blue)" },
    { step: "2", title: isNl ? "Herkenning" : "Recognition", desc: isNl ? "Rekening of niet? Automatisch gefilterd" : "Bill or not? Automatically filtered", icon: <ScanSearch size={16} />, color: "var(--amber)" },
    { step: "3", title: isNl ? "Data extractie" : "Data extraction", desc: isNl ? "Bedrag, IBAN en deadline eruit gehaald" : "Amount, IBAN and deadline extracted", icon: <Zap size={16} />, color: "var(--purple)" },
