    ...
    <div className="bg-[var(--bg)]">
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 pb-16 sm:pb-24">
        {features.map((feature, i) => {
          const isEven = i % 2 === 0;
          return (
            <div key={i} className={`flex flex-col gap-8 py-12 border-b border-[var(--border)] last:border-b-0 sm:flex-row sm:items-center ${isEven ? "" : "sm:flex-row-reverse"}`}>
              {/* Text */}
              ...
              {/* IMAGE PLACEHOLDER: Replace with feature screenshot/mockup */}
              <div className="flex-1 flex justify-center">
                <div className="w-full max-w-sm h-56 sm:h-64 rounded-2xl border-2 border-dashed border-[var(--border)] bg-[var(--surface)] flex flex-col items-center justify-center gap-3">
                  ...
                  <span className="text-xs text-[var(--muted)] opacity-50">{feature.title} — screenshot</span>
                </div>
              </div>
            </div>
          );
