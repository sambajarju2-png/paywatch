...
export async function POST(request: Request) {
  try {
    ...
    /* Save to Supabase — use anon key (RLS allows public inserts) */
    ...
    if (supabaseUrl && supabaseKey) {
      ...
      if (dbError) {
        console.error("Supabase contact insert error:", JSON.stringify(dbError));
      }
    } else {
      console.error("Missing Supabase env vars: NEXT_PUBLIC_SUPABASE_URL or ANON_KEY");
    }
