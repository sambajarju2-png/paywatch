...
const PLAN_META: Record<string, { label: string; badge: string; price: string }> = {
  gratis:          { label: "Gratis",         badge: "bg-gray-100 text-gray-600",    price: "€0" },
  pro_monthly:     { label: "Pro (maand)",    badge: "bg-blue-50 text-blue-600",     price: "€4/mo" },
  pro_yearly:      { label: "Pro (jaar)",     badge: "bg-blue-50 text-blue-600",     price: "€40/jr" },
  premium_monthly: { label: "Premium (maand)",badge: "bg-violet-50 text-violet-600", price: "€8/mo" },
  premium_yearly:  { label: "Premium (jaar)", badge: "bg-violet-50 text-violet-600", price: "€80/jr" },
};

const FEATURES = [
  { key: "ai_insights_enabled",    label: "AI Inzichten",         desc: "Persoonlijke financiële inzichten" },
  { key: "ai_chat_enabled",        label: "AI Chat",              desc: "PayBuddy chatbot" },
...
function formatLetters(n: number) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {[
          { label: "Totaal gebruikers",  value: totalUsers,                                                           color: "#0A2540" },
          { label: "Betaalde gebruikers",value: totalPaying,                                                          color: "#2563EB" },
          { label: "Conversie",          value: totalUsers > 0 ? `${Math.round(totalPaying/totalUsers*100)}%` : "0%", color: "#059669" },
          { label: "MRR (schatting)",    value: `€${(revenue/100).toFixed(0)}`,                                       color: "#7C3AED" },
        ].map(k => (
      ...
      {loading ? (
        <div className="grid grid-cols-5 gap-3">
          {[0,1,2,3,4].map(i => (
            <div key={i} className="bg-white rounded-2xl border border-[#E2E8F0] p-5 h-96 animate-pulse" />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-5 gap-3">
          {rules.map(rule => {
            ...
            return (
              <div key={rule.plan_id} className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm flex flex-col">
                {/* Plan header — clean, no colored border */}
                <div className="p-4 border-b border-[#F1F5F9]">
                  <div className="flex items-center justify-between mb-2">
                    <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${meta.badge}`}>{meta.label}</span>
                    <span className="text-[12px] font-bold text-[#0A2540]">{meta.price}</span>
                  </div>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-[26px] font-extrabold text-[#0A2540] leading-none">{count}</span>
                    <span className="text-[11px] text-[#94A3B8]">gebruikers</span>
                  </div>
                </div>
                ...
                <div className="p-4 border-b border-[#F1F5F9] space-y-3">
                  {/* Voice */}
                  <div>
                    <label className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-wider block mb-1">PayBuddy</label>
                    <select
                  ...
                  {/* Chat */}
                  <div>
                    <label className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-wider block mb-1">Chat/dag</label>
                    <select
                  ...
                  {/* Bezwaarschriften */}
                  <div>
                    <label className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-wider block mb-1">Bezwaarbrieven/mo</label>
                    <select
                      value={rule.dispute_letters_per_month ?? -1}
                      onChange={e => updateNumber(rule.plan_id, "dispute_letters_per_month", parseInt(e.target.value))}
                      disabled={saving?.startsWith(rule.plan_id)}
                      className="w-full text-[12px] font-semibold px-2.5 py-1.5 rounded-xl border border-[#E2E8F0] outline-none bg-[#F8FAFC] text-[#0A2540] focus:border-[#2563EB]"
                    >
                ...
                <div className="p-4 flex-1 space-y-2">
                  {FEATURES.map(f => {
                    ...
                    return (
                      ...
                      >
                        <span className={`text-[11px] font-medium ${enabled ? "text-[#0A2540]" : "text-[#CBD5E1]"}`}>{f.label}</span>
